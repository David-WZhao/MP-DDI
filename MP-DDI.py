...
def main(args):
    ...
    for m in range(nfold):
        ave_aupr += best_aupr[m - 1]
        ave_auc += best_auc[m - 1]
    ave_aupr = format(ave_aupr / nfold, '.4f')
    ave_auc = format(ave_auc / nfold, '.4f')
    print(nfold - 1, "折交叉验证的aupr的平均值为:", ave_aupr)
    print(nfold - 1, "折交叉验证的auc的平均值为:", ave_auc)
